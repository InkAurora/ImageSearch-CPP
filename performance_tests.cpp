...
namespace {
    bool GenerateTestImage(const char* filename, int width, int height) {
        ...
        bool saved = FreeImage_Save(FIF_PNG, bitmap, filename) == TRUE;
        std::cout << (saved ? "Successfully created test image: " : "Failed to save test image: ")
                  << filename << std::endl;
...
int GenerateTestImages() {
    FreeImage_Initialise();

    bool _small = GenerateTestImage("test_image_small.png", 100, 100);
    bool medium = GenerateTestImage("test_image_medium.png", 400, 300);
    bool large = GenerateTestImage("test_image_large.png", 800, 600);

    FreeImage_DeInitialise();
    return (_small && medium && large) ? 0 : 1;
}
...
int RunPerformanceTests() {
    try {
        ...
        if (GenerateTestImages() != 0) {
            std::cerr << "Unable to generate test images." << std::endl;
            return 1;
        }
